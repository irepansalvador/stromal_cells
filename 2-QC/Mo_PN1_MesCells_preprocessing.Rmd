...
In the example below, we visualize QC metrics, and use these to filter cells.
  ...
  We filter cells that have unique feature counts over 6,000 or less than 400
...
Next, we apply a linear transformation ('scaling') that is a standard pre-processing step prior to dimensional reduction techniques like PCA. The ScaleData function:
  ...
  The results of this are stored in Mo_PN1_MesCells[["RNA"]]@scale.data
...
seurat2shiny(object = Mo_PN1_MesCells, assay = "RNA",save = T,
             name = "rds_viz/Mo_PN1_MesCells_preprocess")


```


## Make a list of gene markers for each cluster

```{r,eval=FALSE}
all.markers <- FindAllMarkers(object = Mo_PN1_MesCells, only.pos = T, 
                              logfc.threshold = 0.5, min.diff.pct = 0.2)

write.csv(x = all.markers, file ="Mo_PN1_MesCells_allmarkers.csv")
```
